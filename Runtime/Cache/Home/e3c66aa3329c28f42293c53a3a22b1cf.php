...
$(".btn1").click(function (){
  location.replace("/index.php/home/search/searchGoods2?table_id="+$(".layui-input").val());
});
      ...
      dropdown.render({
        ...
        ,click: function(menudata){
          if(menudata.id === 'del'){
            ...
            layer.open({
          type: 1, 
          content: '<div style="margin: 40px"> <p style="margin-top: 30px;">名称：<input disabled type="text" class="name" value="'+data.name+'"></p><p style="margin-top: 30px;">名称：<input disabled type="text" class="goods_id " value="'+data.goods_id+'"></p><p style="margin-top: 30px;">规格：<input  disabled type="text" class="norms" value="'+data.norms+'"></p><p style="margin-top: 30px;">出库数量：<input type="text" class="numbers" value="" placeholder="请输入出库数量"></p><p style="margin-top: 30px;">单位：<input type="text" class="unit" value="'+data.unit+'"></p><p style="margin-top: 30px;">分类：<input disabled type="text" class="type" value="'+data.type+'"></p><p style="margin-top: 30px;">备注：<input disabled type="text" class="remark" value="'+data.remark+'"></p></div><p style="padding-left: 200px;margin-bottom: 40px;"><button class="save" style="background-color: green; color: white;width: 60px; border:1px solid gray;cursor: pointer" >保存</button></p>' 
      });

     
          $(".save").click(function(){
             if(parseInt($(".numbers").val()) > parseInt(data2.numbers)){
                layer.msg("您输入的出库数量大于当前库存，请重新输入！");
             }else{
                $.ajax({
                type: "GET",
                url: "/index.php/home/goodsOut/out",
                ...
                name: $(".name").val(),
                norms: $(".norms").val(),
                numbers: parseInt(data2.numbers)- parseInt($(".numbers").val()),
                unit:$(".unit").val(),
                type:$(".type").val(),
                remark:$(".remark").val()
                      });
