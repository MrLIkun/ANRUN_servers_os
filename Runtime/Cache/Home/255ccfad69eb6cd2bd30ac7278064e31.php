...
$(".layui-btn").click(function (){
	location.replace("/index.php/home/goodsOut/outrec?table_id="+$(".layui-input").val());
});
layui.use('table', function(){
  ...
  table.render({
    ...
    ,cols: [[ //标题栏
      ...
      ,{field: 'type', title: '分类', }
      ,{field: 'change_time', title: '出库时间', sort: true}
      ,{field: 'remark', title: '备注'}
      ,{field: 'in_people', title: '出库人员'}
    ]]
      ...
      dropdown.render({
        ...
        ,click: function(menudata){
          if(menudata.id === 'del'){
            ...
            layer.open({
  				type: 1, 
  				content: '<div style="margin: 40px"> <p style="margin-top: 30px;">名称：<input type="text" class="name" value="'+data.name+'"></p><p style="margin-top: 30px;">规格：<input type="text" class="norms" value="'+data.norms+'"></p><p style="margin-top: 30px;">数量：<input type="text" class="numbers" value="'+data.numbers+'"></p><p style="margin-top: 30px;">单位：<input type="text" name="unit"  class="unit" value="'+data.unit+'"></p><p style="margin-top: 30px;">分类：<input type="text" class="type" value="'+data.type+'"></p><p style="margin-top: 30px;">备注：<input type="text" class="remark" value="'+data.remark+'"></p></div><p style="padding-left: 200px;margin-bottom: 40px;"><button class="save" style="background-color: green; color: white;width: 60px; border:1px solid gray;cursor: pointer" >保存</button></p>' 
			});
			$(".save").click(function(){
					 $.ajax({
              			type: "GET",
